...
<div class="flex space-x-6 h-24">
    ...
    <div class="flex flex-col space-y-1">
        <div class="text-2xl lg:text-3xl font-bold">{{ __($title) }}</div>
        <div class="text-base lg:text-xl font-semibold">{{ $clan->alias }} | {{ $clan->name }}</div>
        <div class="text-xs lg:text-sm italic">{{ $clan->description ?? __('hll.clans.show.no_description') }}</div>
    </div>
</div>
