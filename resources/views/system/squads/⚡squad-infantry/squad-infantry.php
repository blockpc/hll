...
new class extends Component
{
    ...
    {
        $squad = $this->roster->infantrySquads()->findOrFail($squadId);
        if ($squad->isFull()) {
            $this->alert(__('hll.squads.squad_infantry.full_squad'), 'warning', __('hll.squads.squad_infantry.title'));
    ...
    public function remove_soldier(int $soldierId): void
    {
        ...
        if ($sodier) {
            $sodier->delete();
            $this->reRender();
            $this->alert(__('hll.squads.soldier_removed', ['name' => $name]), 'success', __('hll.squads.squad_infantry.title'));
        }
    }
};
