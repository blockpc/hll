<?php

return [
    'permissions' => [
        'menu' => 'Permisos',
        'title' => 'Listado de permisos',
        'description' => 'Muestra los permisos que pueden ser asignados a los roles y/o usuarios.',
        'search_permissions' => 'Buscar permisos...',
        'select_key' => 'Seleccionar clave',
        '403' => [
            'permissions-index' => 'No tienes permiso para ver la lista de permisos.',
            'permissions-edit' => 'No tienes permiso para editar permisos.',
        ],
        'table' => [
            'name' => 'Nombre',
            'description' => 'Descripción',
            'key' => 'Clave',
            'type' => 'Tipo',
            'actions' => 'Acciones',
            'no_permissions' => 'No hay permisos registrados.',
            'edit' => 'Editar',
        ],
        'edit' => [
            'title' => 'Editar permiso',
            'display_name' => 'Nombre',
            'description' => 'Descripción',
            'save' => 'Guardar cambios',
            'validations' => [
                'super_admin_permission_error' => 'Uno de los permisos seleccionados no es válido.',
                'display_name.required' => 'El nombre es obligatorio.',
                'display_name.string' => 'El nombre debe ser una cadena de texto.',
                'display_name.max' => 'El nombre no puede tener más de 255 caracteres.',
                'description.required' => 'La descripción es obligatoria.',
                'description.string' => 'La descripción debe ser una cadena de texto.',
                'description.max' => 'La descripción no puede tener más de 255 caracteres.',
            ],
        ],
        'keys' => [
            'sudo' => 'Super Admin',
            'users' => 'Usuarios',
            'roles' => 'Roles',
            'permissions' => 'Permisos',
            'clans' => 'Clanes',
        ],
    ],
    'roles' => [
        'menu' => 'Roles',
        'title' => 'Listado de roles',
        'subtitle' => 'Muestra los roles que pueden ser asignados a los usuarios.',
        'search_roles' => 'Buscar roles...',
        'back_to_table' => 'Volver al listado de roles',
        'super_admin_permission_error' => 'Uno de los permisos seleccionados no es válido.',
        'name_already_exists' => 'Ya existe un rol con ese nombre.',
        '403' => [
            'roles-index' => 'No tienes permiso para ver la lista de roles.',
            'roles-create' => 'No tienes permiso para crear roles.',
            'roles-edit' => 'No tienes permiso para editar roles.',
            'roles-delete' => 'No tienes permiso para eliminar roles.',
        ],
        'table' => [
            'name' => 'Nombre',
            'description' => 'Descripción',
            'key' => 'Clave',
            'actions' => 'Acciones',
            'no_roles' => 'No hay roles registrados.',
            'edit' => 'Editar',
        ],
        'buttons' => [
            'create' => 'Crear nuevo rol',
            'edit' => 'Editar rol',
            'delete' => 'Eliminar rol',
            'back' => 'Volver al listado de roles',
        ],
        'edit' => [
            'title' => 'Editar rol',
            'subtitle' => 'Modifica los detalles del rol.',
            'display_name' => 'Nombre',
            'description' => 'Descripción',
            'permissions' => 'Permisos',
            'save' => 'Guardar cambios',
            'success_message' => 'El rol se ha actualizado exitosamente.',
            'form' => [
                'display_name' => 'Ingresa el nombre del rol.',
                'description' => 'Proporciona una descripción para el rol.',
                'select_permissions' => 'Selecciona los permisos que deseas asignar a este rol.',
            ],
            'validations' => [
                'display_name.required' => 'El nombre es obligatorio.',
                'display_name.string' => 'El nombre debe ser una cadena de texto.',
                'display_name.max' => 'El nombre no puede tener más de 255 caracteres.',
                'description.required' => 'La descripción es obligatoria.',
                'description.string' => 'La descripción debe ser una cadena de texto.',
                'description.max' => 'La descripción no puede tener más de 255 caracteres.',
            ],
        ],
        'create' => [
            'title' => 'Crear nuevo rol',
            'subtitle' => 'Rellena los campos para crear un nuevo rol.',
            'display_name' => 'Nombre',
            'description' => 'Descripción',
            'permissions' => 'Permisos',
            'save' => 'Crear rol',
            'form' => [
                'display_name' => 'Ingresa el nombre del rol.',
                'description' => 'Proporciona una descripción para el rol.',
                'select_permissions' => 'Selecciona los permisos que deseas asignar a este rol.',
            ],
            'validations' => [
                'display_name.required' => 'El nombre es obligatorio.',
                'display_name.string' => 'El nombre debe ser una cadena de texto.',
                'display_name.max' => 'El nombre no puede tener más de 255 caracteres.',
                'description.required' => 'La descripción es obligatoria.',
                'description.string' => 'La descripción debe ser una cadena de texto.',
                'description.max' => 'La descripción no puede tener más de 255 caracteres.',
            ],
            'success_message' => 'El rol se ha creado exitosamente.',
        ],
        'delete' => [
            'title' => 'Eliminar Rol',
            'button' => 'Sí, quiero eliminar este rol',
            'role_name' => 'Nombre del rol',
            'password' => 'Contraseña',
            'confirmation_message' => '¿Estás seguro de que deseas eliminar este rol? Esta acción no se puede deshacer. Escriba :name y su contraseña para confirmar.',
            'success_message' => 'El rol se ha eliminado exitosamente.',
            'no_role_selected' => 'No se ha seleccionado ningún rol para eliminar.',
            'cannot_delete_role' => 'El rol seleccionado no se puede eliminar.',
            'invalid_role_name' => 'El nombre del rol no coincide.',
        ],
    ],
    'users' => [
        'menu' => 'Usuarios',
        'title' => 'Listado de usuarios',
        'subtitle' => 'Muestra los usuarios registrados en el sistema.',
        'search_users' => 'Buscar usuarios...',
        'back_to_table' => 'Volver al listado de usuarios',
        '403' => [
            'users-index' => 'No tienes permiso para ver la lista de usuarios.',
            'users-create' => 'No tienes permiso para crear usuarios.',
            'users-edit' => 'No tienes permiso para editar usuarios.',
            'users-delete' => 'No tienes permiso para eliminar usuarios.',
            'invalid-verification-link' => 'El enlace de verificación no es válido.',
        ],
        'buttons' => [
            'create' => 'Crear nuevo usuario',
            'edit' => 'Editar usuario',
            'delete' => 'Eliminar usuario',
        ],
        'table' => [
            'status' => 'Estado',
            'name' => 'Nombre',
            'email' => 'Correo electrónico',
            'roles' => 'Roles',
            'permissions' => 'Permisos',
            'actions' => 'Acciones',
            'no_users' => 'No hay usuarios registrados.',
            'edit' => 'Editar',
            'no_actions' => 'No hay acciones disponibles',
            'verified' => 'Verificado',
            'not_verified' => 'No verificado',
        ],
        'delete' => [
            'title' => 'Eliminar Usuario',
            'button' => 'Sí, quiero eliminar este usuario',
            'user_name' => 'Nombre del usuario',
            'password' => 'Contraseña',
            'confirmation_message' => '¿Estás seguro de que deseas eliminar este usuario? Esta acción no se puede deshacer. Escriba :name y su contraseña para confirmar.',
            'success_message' => 'El usuario se ha eliminado exitosamente.',
            'no_user_selected' => 'No se ha seleccionado ningún usuario para eliminar.',
            'cannot_delete_user' => 'El usuario seleccionado no se puede eliminar.',
            'invalid_user_name' => 'El nombre del usuario no coincide.',
            'cannot_delete_yourself' => 'No puedes eliminar tu propio usuario.',
            'cannot_delete_sudo' => 'No puedes eliminar a un super usuario.',
        ],
        'create' => [
            'title' => 'Crear nuevo usuario',
            'subtitle' => 'Rellena los campos para crear un nuevo usuario.',
            'name' => 'Nombre',
            'email' => 'Correo electrónico',
            'password' => 'Contraseña',
            'confirm_password' => 'Confirmar contraseña',
            'send_email' => 'Enviar correo de notificación al usuario',
            'save' => 'Crear usuario',
            'general' => [
                'title' => 'Datos del Usuario',
                'description' => 'Completa el nombre y el correo del usuario.',
            ],
            'passwords' => [
                'title' => 'Asociar Password',
                'description' => 'Selecciona una contraseña para el usuario.',
                'auto_generate_label' => '¿Quieres que el sistema genere un password automático?',
            ],
            'roles' => [
                'title' => 'Asignar Roles',
                'description' => 'Selecciona los roles que deseas asignar a este usuario.',
            ],
            'permissions' => [
                'title' => 'Asignar Permisos',
                'description' => 'Selecciona los permisos que deseas asignar a este usuario.',
            ],
            'form' => [
                'name' => 'Ingresa el nombre del usuario.',
                'email' => 'Proporciona el correo electrónico del usuario.',
                'password' => 'Establece una contraseña para el usuario.',
                'password_confirmation' => 'Confirma la contraseña ingresada.',
                'select_roles' => 'Selecciona los roles que deseas asignar a este usuario.',
                'select_permissions' => 'Selecciona los permisos que deseas asignar a este usuario.',
            ],
            'validations' => [
                'name.required' => 'El nombre es obligatorio.',
                'name.string' => 'El nombre debe ser una cadena de texto.',
                'name.max' => 'El nombre no puede tener más de 255 caracteres.',
                'email.required' => 'El correo electrónico es obligatorio.',
                'email.string' => 'El correo electrónico debe ser una cadena de texto.',
                'email.email' => 'El correo electrónico debe ser una dirección de correo válida.',
                'email.max' => 'El correo electrónico no puede tener más de 255 caracteres.',
                'email.unique' => 'Ya existe un usuario con ese correo electrónico.',
                'password.required' => 'La contraseña es obligatoria.',
                'password.string' => 'La contraseña debe ser una cadena de texto.',
                'password.min' => 'La contraseña debe tener al menos 8 caracteres.',
                'password.confirmed' => 'La confirmación de la contraseña no coincide.',
            ],
            'success_message' => 'El usuario \':name\' se ha creado exitosamente.',
            'creation_error_message' => 'El usuario no pudo ser creado. Comunicate con el administrador.',
            'email_error_message' => 'El usuario se ha creado, pero no se pudo enviar el correo de notificación.',
        ],
        'edit' => [
            'title' => 'Editar usuario',
            'subtitle' => 'Modifica los detalles del usuario.',
            'name' => 'Nombre',
            'email' => 'Correo electrónico',
            'password' => 'Contraseña',
            'confirm_password' => 'Confirmar contraseña',
            'save' => 'Guardar cambios',
            'general' => [
                'title' => 'Datos del Usuario',
                'description' => 'Modifica el nombre y el correo del usuario.',
            ],
            'passwords' => [
                'title' => 'Asociar Password',
                'description' => 'Selecciona una nueva contraseña para el usuario o deja los campos vacíos para mantener la contraseña actual.',
                'auto_generate_label' => '¿Quieres que el sistema genere un password automático?',
            ],
            'user_email_verification' => [
                'title' => 'Verificación de correo',
                'description' => 'Estado de la verificación de correo del usuario.',
                'verified' => 'El correo de este usuario ha sido verificado.',
                'not_verified' => 'El correo de este usuario no ha sido verificado.',
                'email_verified_success_message' => 'El correo del usuario se ha verificado exitosamente.',
                'email_unverified_success_message' => 'El correo del usuario se ha marcado como no verificado exitosamente.',
                'mark_as_verified' => 'Marcar como verificado',
                'mark_as_unverified' => 'Marcar como no verificado',
                'resend_verification_email' => 'Reenviar correo de verificación',
                'success_resend_verification_email' => 'El correo de verificación se ha reenviado exitosamente.',
                'error_email_change_resend' => 'No se pudo reenviar el correo de verificación.',
            ],
            'roles' => [
                'title' => 'Asignar Roles',
                'description' => 'Selecciona los roles que deseas asignar a este usuario.',
            ],
            'permissions' => [
                'title' => 'Asignar Permisos',
                'description' => 'Selecciona los permisos que deseas asignar a este usuario.',
            ],
            'form' => [
                'name' => 'Ingresa el nombre del usuario.',
                'email' => 'Proporciona el correo electrónico del usuario.',
                'password' => 'Establece una nueva contraseña para el usuario o deja el campo vacío para mantener la contraseña actual.',
                'password_confirmation' => 'Confirma la nueva contraseña ingresada.',
                'select_roles' => 'Selecciona los roles que deseas asignar a este usuario.',
                'select_permissions' => 'Selecciona los permisos que deseas asignar a este usuario.',
            ],
            'validations' => [
                'name.required' => 'El nombre es obligatorio.',
                'name.string' => 'El nombre debe ser una cadena de texto.',
                'name.max' => 'El nombre no puede tener más de 255 caracteres.',
                'email.required' => 'El correo electrónico es obligatorio.',
                'email.string' => 'El correo electrónico debe ser una cadena de texto.',
                'email.email' => 'El correo electrónico debe ser una dirección de correo válida.',
                'email.max' => 'El correo electrónico no puede tener más de 255 caracteres.',
                'email.unique' => 'Ya existe un usuario con ese correo electrónico.',
                'password.string' => 'La contraseña debe ser una cadena de texto.',
                'password.min' => 'La contraseña debe tener al menos 8 caracteres.',
                'password.confirmed' => 'La confirmación de la contraseña no coincide.',
            ],
            'success_message' => 'El usuario \':name\' se ha actualizado exitosamente.',
            'clans' => [
                'title' => 'Clanes del usuario',
                'description' => 'Muestra los clanes a los que pertenece el usuario.',
                'owned_clan' => 'Clan que posee el usuario',
                'no_owned_clan' => 'El usuario no posee ningún clan.',
                'clans_list' => 'Lista de clanes a los que pertenece el usuario',
                'no_clans' => 'El usuario no pertenece a ningún clan.',
                'assign_clan' => 'Asignar clan al usuario',
                'search_clan' => 'Buscar clan...',
                'select_clan' => 'Seleccionar clan',
                'remove_clan' => 'Eliminar clan del usuario',
                'success_assign_clan' => 'El clan :name se ha asignado al usuario exitosamente.',
                'success_remove_clan' => 'El clan se ha eliminado del usuario exitosamente.',
                'error_assign_clan' => 'No se pudo asignar el clan al usuario.',
                'error_remove_clan' => 'No se pudo eliminar el clan del usuario.',
            ],
        ],
        'mails' => [
            'new_user_created_subject' => '¡Bienvenido a :name!',
            'new_user_created_message' => 'Tu cuenta ha sido creada exitosamente. Puedes iniciar sesión utilizando el siguiente enlace:',
            'new_user_verify_message' => 'Antes de iniciar sesión, verifica tu correo electrónico con el siguiente enlace:',
            'new_user_verify_button' => 'Verificar correo electrónico',
            'new_user_created_login_button' => 'Iniciar sesión',
            'new_user_change_password' => 'Cambiar Contraseña',
            'new_user_created_recommendation' => 'Te recomendamos cambiar tu contraseña antes de iniciar sesión por primera vez.',
            'new_user_created_farewell' => '¡Gracias por unirte a nosotros!',
            'new_user_created_signature' => 'El equipo de :app',
            'new_user_created_disclaimer' => 'Si no has creado esta cuenta, puedes ignorar este correo electrónico.',
            'resend' => [
                'verification_email_subject' => 'Reenvío de correo de verificación de cuenta',
                'verification_email_greeting' => '¡Saludos :name!',
                'verification_email_message' => 'Se ha solicitado el reenvío del correo de verificación para tu cuenta. Puedes verificar tu correo electrónico utilizando el siguiente enlace:',
                'verification_recommendation' => 'Te recomendamos cambiar tu contraseña antes de volver a iniciar sesión.',
                'verification_email_button' => 'Verificar correo electrónico',
                'verification_email_farewell' => '¡Gracias por ser parte de nuestra comunidad!',
                'verification_email_signature' => 'El equipo de :app',
                'verification_email_disclaimer' => 'Si no has solicitado este correo, puedes ignorar este mensaje.',
            ],
        ],
    ],
    'notifications' => [
        'menu' => 'Notificaciones',
        'from' => 'De',
        'title' => 'Notificaciones de Usuario',
        'title_label' => 'Titulo',
        'title_placeholder' => 'Titulo del mensaje',
        'content_label' => 'Contenido',
        'content_placeholder' => 'Contenido de la notificación',
        'notifications_all' => 'Todas las notificaciones',
        'notifications_unread' => 'Notificaciones no leídas',
        'notifications_read' => 'Notificaciones leídas',
        'counter' => '{0} No tienes notificaciones|{1} Tienes 1 notificación|[2,*] Tienes :count notificaciones',
        'description' => 'Administra las notificaciones que recibes en el sistema.',
        'mark_all_as_read' => 'Marcar todas como leídas',
        'no_notifications' => 'No tienes notificaciones.',
        'view_all' => 'Ver todas las notificaciones',
        'select_user' => 'Seleccionar usuario',
        'max_64_characters' => 'Máximo 64 caracteres.',
        'max_255_characters' => 'Máximo 255 caracteres.',
        'send_notification_to_user' => 'Enviar notificación a usuario',
        'mark_as_read' => 'Marcar como leída',
        'mark_as_unread' => 'Marcar como no leída',
        'read_at' => 'Leída el :date',
    ],
];
