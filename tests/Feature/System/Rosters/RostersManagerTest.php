...
it('does not allow duplicate slug in the same clan', function () {
    $owner = new_user();
    $clan = new_clan($owner);
    $slug = fake()->slug();
...
it('allows duplicate slug in different clans', function () {
    $ownerA = new_user();
    $ownerB = new_user();
    ...
    expect($rosterA->slug)->toBe($slug);
    expect($rosterB->slug)->toBe($slug);
...
it('redirects guest to login for public roster table URL', function () {
    ...
    $response = $this->get(route('rosters.table', ['clan' => $clan->alias, 'roster' => $roster->slug]));
